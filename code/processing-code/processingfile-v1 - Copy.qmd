...
data_location <- here::here("JoeDainis-ready-exercise","data","raw-data","exampledata2.xlsx")
...
Now we see that there is another NA, but it's not `NA` from R, instead it was loaded as character and is now considered as a category. We'll proceed here by removing that individual with that NA entry. Since this keeps an empty category for Gender, I'm also using droplevels() to get rid of it.
...
Cleaning new variables added Jellybeans_in_pocket and Food_lifestyle

```{r}
# need to add readr package 
library(readr)
processeddata <- d4 %>%
  # Convert Jellybeans_in_pocket to numeric
  mutate(
    Jellybeans_in_pocket = parse_number(Jellybeans_in_pocket)
  ) %>%
  
  # Remove rows with missing jellybean counts
  filter(!is.na(Jellybeans_in_pocket)) %>%
  
  # Keep only realistic values (≤ 10,000)
  filter(Jellybeans_in_pocket <= 10000) %>%
  
  # Make sure Food_lifestyle is a factor
  mutate(
    Food_lifestyle = factor(Food_lifestyle)
  )
  # check data 
...
```{r}
save_data_location <- here::here("JoeDainis-ready-exercise","data","processed-data","processeddata.rds")
